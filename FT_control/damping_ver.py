...
def load_config(config_path: Path) -> AppConfig:
    ...
    if config.control.damping <= 0.0:
        raise ValueError('control.damping.damping must be positive')
    return config
...
def has_force_input(ft_comp, force_threshold):
    return max(abs(ft_comp[i]) for i in range(3)) > force_threshold


def compute_damping_step(
    ft_comp,
    dt,
    control_config: ControlConfig,
):
    command_velocity = [ft_comp[i] / control_config.damping for i in range(6)]
